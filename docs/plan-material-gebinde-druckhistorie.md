...
### Review nach dem S3-Ausbau

| Befund                                                                                                                                               | Entscheidung                                                                                                                                                                                                       |
| ---------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Das Zeitlimit von 30 s griff nie: `AwsClient.fetch` verpackt die Anfrage in ein eigenes `Request`, dem undici das Abbruchsignal nur schwach zuordnet | **Behoben**: Signieren mit `AwsV4Signer`, Senden mit `fetch` und dem Signal direkt, eigene Wiederholung. Test mit einem Speicher, der vor den Kopfzeilen bzw. mitten im Körper hängt – rot gegen die alte Fassung. |
| Das Zeitlimit endete nach den Kopfzeilen, auch wo der Körper noch gelesen wird (Liste, Vorschau, Fehlertext)                                         | **Behoben**: Es gilt bis zum Ende der Verarbeitung; nur `open` gibt es nach den Kopfzeilen frei.                                                                                                                   |
| Fehlende `content-length` ergab Größe 0 – der Download wäre abgeschnitten                                                                            | **Behoben**: Ohne verlässliche Länge (chunked, komprimiert) wird gepuffert. Test mit einer chunked-Antwort.                                                                                                        |
| Verworfene Antworten bei Wiederholungen hielten ihre Verbindung                                                                                      | **Behoben**: vor dem nächsten Versuch geschlossen.                                                                                                                                                                 |
| `S3_PREFIX=../` verließ den Bucket (die Adresse löst `..` auf)                                                                                       | **Behoben**: `.`- und `..`-Abschnitte abgelehnt.                                                                                                                                                                   |
| Bucket mit Punkt im Host-Stil: TLS scheitert bei AWS                                                                                                 | **Behoben**: Vorgabe dann Pfad-Stil.                                                                                                                                                                               |
| `ListBucket` mit Präfix-Bedingung: AWS antwortet 403 statt 404                                                                                       | **Dokumentiert** (README): `ListBucket` ohne Bedingung. Ein 403 als „nicht da“ zu lesen, verdeckte falsche Zugangsdaten.                                                                                           |
| Umzug kopiert `.tmp`-Reste mit, die der S3-Treiber nie aufräumt                                                                                      | **Dokumentiert**: `--exclude "*.tmp"` im README.                                                                                                                                                                   |     | Der Lauf gegen einen echten Speicher räumte mit leerem Präfix **alle** Dateien der App im Bucket ab (beim Probelauf passiert: die Fotos der lokalen Test-App) | **Behoben**: Die Tests löschen nur, was sie selbst angelegt haben; AGENTS.md rät zu einem eigenen Test-Bucket. |
